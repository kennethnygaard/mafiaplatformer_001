...
func _ready():
	rnd.randomize()
	$AnimationPlayer.play("walking_nothing")
	...
	$Turn_area.connect("area_exited", self, "on_turn_area_exited")
...
func _process(delta):
	...
	if(is_player_in_sight && player.active_gun > 0):
		has_detected_player = true
	
	if(is_shooting):
		move_vector.x = 0
	else:
		if(is_player_in_sight && has_detected_player):
			shoot()
		else:
			if(is_always_standing_still):
				move_vector.x = 0
				if(is_alive):
					$AnimationPlayer.play("idle_nothing")
			else:
				move_vector.x = move_speed*direction
				if(is_alive):
					$AnimationPlayer.play("walking_nothing")
	if(is_alive && is_player_behind && has_detected_player):
		turn()
...
func on_turn_area_exited(area2d):
...
func on_shoot_timer_timeout():
	if(is_alive):
		if(shooting_direction == "ahead"):
			$AnimationPlayer.play("shooting_handgun")
